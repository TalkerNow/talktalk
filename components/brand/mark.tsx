...
const VIEWBOX = "-682.69 -622.02 1365.38 1365.38";

function SymboleOutline({
  className,
  title,
}: {
  className?: string;
...
export function TalkerMark({ className, title, filled = false }: MarkProps) {
  const Mark = filled ? SymbolePlein : SymboleOutline;
...
export function TalkerWordmark({ compact = false }: { compact?: boolean }) {
  return (
    ...
    >
      <SymboleOutline className="h-[1.05em] w-auto shrink-0" />
